...
from launch.actions import (IncludeLaunchDescription, DeclareLaunchArgument,
                             OpaqueFunction, GroupAction, TimerAction)
...
def launch_setup(context):
    ...
    # 'online' talks to the cloud LLM/ASR APIs, 'offline' uses the on-device models.
    ...
    # The offline speech pipeline needs more time to load its local models before
    # vocal_detect/agent_process are safe to start; online mode only needs a short delay.
    time_interval = 10.0 if asr_mode == 'offline' else 1.0
    large_models_launch = GroupAction(
        actions=[
            tts_node_launch,
            TimerAction(period=time_interval, actions=[vocal_detect_launch]),
            TimerAction(period=time_interval, actions=[agent_process_launch]),
        ]
    )
